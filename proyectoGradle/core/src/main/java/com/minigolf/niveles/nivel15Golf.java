...
public class nivel15Golf {
    ...
    public static HashMap<Body, Boolean> crearNivel(Stage stage, World mundoBox2d, Image imagePuntoDeInicio,
        ...
        aceleracionBodyDef1.position.set(340 * PIXEL_A_METRO, 520* PIXEL_A_METRO);
        ...
        aceleracionFixtureDef1.filter.categoryBits = manejoEventos.CATEGORIA_ACELERA_ABAJO; // Asigna su categoría
        ...
        /* Representacion Visual del Boost */
        Image imageAceleracion1 = new Image(texturaBoost);
        float aceleracionAnchoPx = 2 * mitadAnchoAceleracion1_m / PIXEL_A_METRO; // Ancho del boost
        float aceleracionAltoPx = 2 * mitadAltoAceleracion1_m / PIXEL_A_METRO; // Alto del boost
        ...
        aceleracionBody1.setUserData(imageAceleracion1); // Asociamos la imagen al body para manejoColisiones

        // Zona de Aceleración 2
        BodyDef aceleracionBodyDef2 = new BodyDef();
        aceleracionBodyDef2.position.set(340 * PIXEL_A_METRO, 620 * PIXEL_A_METRO);
        Body aceleracionBody2 = mundoBox2d.createBody(aceleracionBodyDef2);
        hashMapBodiesTemporales.put(aceleracionBody2, false); // No es una bola, es un elemento del nivel   
        PolygonShape aceleracionShape2 = new PolygonShape();
        // setAsBox toma la mitad del ancho y la mitad del alto
        float mitadAnchoAceleracion2_m = 40 * PIXEL_A_METRO;
        float mitadAltoAceleracion2_m = 40 * PIXEL_A_METRO;
        aceleracionShape2.setAsBox(mitadAnchoAceleracion2_m, mitadAltoAceleracion2_m);
        FixtureDef aceleracionFixtureDef2 = new FixtureDef();
        aceleracionFixtureDef2.shape = aceleracionShape2;
        aceleracionFixtureDef2.isSensor = true; // ¡CLAVE! Lo convierte en
        // un sensor (trigger), no en un obstáculo físico
        aceleracionFixtureDef2.filter.categoryBits = manejoEventos.CATEGORIA_ACELERA_ARRIBA; // Asigna su categoría
        aceleracionFixtureDef2.filter.maskBits = manejoEventos.CATEGORIA_BOLA; // Solo interactúa con la bola   
        aceleracionBody2.createFixture(aceleracionFixtureDef2);
        aceleracionShape2.dispose();
        aceleracionBody2.setUserData("zonaAceleracion"); // Identificador para manejoColisiones

        // Representacion Visual del Boost

        Image imageAceleracion2 = new Image(texturaBoost);
        float aceleracion2AnchoPx = 2 * mitadAnchoAceleracion2_m / PIXEL_A_METRO; // Ancho del boost
        float aceleracion2AltoPx = 2 * mitadAltoAceleracion2_m / PIXEL_A_METRO; // Alto del boost
        imageAceleracion2.setSize(aceleracion2AnchoPx, aceleracion2AltoPx);
        float aceleracion2X_px = (aceleracionBodyDef2.position.x / PIXEL_A_METRO) - (aceleracion2AnchoPx / 2);
        float aceleracion2Y_px = (aceleracionBodyDef2.position.y / PIXEL_A_METRO) - (aceleracion2AltoPx / 2);
        imageAceleracion2.setPosition(aceleracion2X_px, aceleracion2Y_px);
        imageAceleracion2.setOrigin(aceleracion2AnchoPx / 2, aceleracion2AltoPx / 2);
        imageAceleracion2.setRotation(180); // Rotamos la imagen 180 grados
        stage.addActor(imageAceleracion2);
        aceleracionBody2.setUserData(imageAceleracion2); // Asociamos la imagen al body para manejoColisiones
        ...
        float paredSuperiorEspejoX_px = (bodyDefParedSuperiorEspejo.position.x / PIXEL_A_METRO) - (paredSuperiorEspejoAnchoPx / 2);
        float paredSuperiorEspejoY_px = (bodyDefParedSuperiorEspejo.position.y / PIXEL_A_METRO) - (paredSuperiorEspejoAltoPx / 2);
        ...
        float paredInferiorEspejoX_px = (bodyDefParedInferiorEspejo.position.x / PIXEL_A_METRO) - (paredInferiorEspejoAnchoPx / 2);
        float paredInferiorEspejoY_px = (bodyDefParedInferiorEspejo.position.y / PIXEL_A_METRO) - (paredInferiorEspejoAltoPx / 2);
